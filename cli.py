...
"Added by E.R. to process multiple generic files"
@cli.command('process-generic-folder')
...
def process_generic_folder(infile, station_id, out_folder):
    """Process a generic netCDF input file into a FOWD output file."""
    from .generic_source import process_folder
    from .logs import setup_file_logger
    ...
    try:
        process_folder(infile, out_folder, station_id=station_id)
    except Exception:
        click.echo('Error during processing', err=True)
        raise
    else:
...
"Added by E.R. to process multiple generic files in parallel"
@cli.command('process-generic-folder-parallel')
@click.argument('INFILE', type=click.Path(file_okay=False, readable=True, exists=True))
@click.option(
    '-o', '--out-folder',
    type=click.Path(file_okay=False, writable=True),
...
def process_generic_folder_parallel(infile, station_id, out_folder):
    """Process a generic netCDF input file into a FOWD output file."""
    from .generic_source import process_folder_parallel
    from .logs import setup_file_logger
    ...
    try:
        process_folder_parallel(infile, out_folder, station_id=station_id)
    except Exception:
        click.echo('Error during processing', err=True)
        raise
    else:
